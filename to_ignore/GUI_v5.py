# Most recent GUI
# Graphical Automated Electrophysiology analysis
import os
import sys
from tkinter import *
...
from analysis.run_model_v5 import run_analysis
...
class data_to_use:
    ...
    def run_electro_analysis(self):
        ...
        stop_button.grid(row=25, column=3, pady = (0,10),padx=(10,0), sticky = 'w')
        
        # Set input folder, output folder, file name
        # ---------- Files Processed Heading ------------------------------------------
        files_processed_titles = ttk.Label(root, text="File status:", style = 'MidHeader.TLabel')
        files_processed_titles.grid(row=2, column=3, padx=(0,10), sticky = W)
        # ---------- Files Processed List------------------------------------------
        frame = ttk.Frame(root)
        frame.grid(row=3, column=3, padx=(0, 20), sticky="nwe", rowspan=17)
        ...
        for file in enumerate(dir_list):
            ...
            #! TEMP - Ned to store all returns here, and if ANY of them is False, then stop with errors
        ...
        trigger_calculations_button.grid(row=25, column=3, pady = (0,10),padx=(10,0), sticky = 'w')
...
class ImageHovertip(Hovertip):
    ...
    def showcontents(self):
        label = Label(self.tipwindow, image=self.image, bg="white")
...
def trigger_calculations():
    ...
    try:        
        ...
        # Class method, to view the data in cmd.
    ...
    # #? If not all configuration is done and Validation failed
        ...
        error_text.grid(row=error_text_row, column=3, padx=(0,10), sticky = W)
...
def get_and_validate_file_path():
    ...
    The first folder is where the code resides, but it remembers thte last used location
    """   
    # Staring directory - where the code lives
...
def validate_input_folder()-> bool:
    ...
    # If the dialog wasnt even opened
    else:
...
def remove_status_and_errors() -> None:
    """
    Remove Error messages and remove status and status updates
    """
    # Remove any remaining errors
    error_text.grid_forget()

    # Loops through all col 4 entries, and deletes them ones besides the title
    for label in root.grid_slaves():
        if int(label.grid_info()["column"]) == 3 and \
        int(label.grid_info()["row"]) >= 2 and \
        int(label.grid_info()["row"]) <= 5:
            label.grid_forget()

    # Remove Process update
    status_title.config(text = '', foreground='black')
...
#? Desperate attempt of fixing the scailing
# ----------------------------------------------------------------------------------------------------
# original_dpi = root.winfo_fpixels('1i')   # This is the DPI of the computer you're making/testing the script on.
...
init_gui_style(root)
root.grid_columnconfigure(0, weight=2, uniform="sample")
root.grid_columnconfigure(1, weight=2, uniform="sample")
root.grid_columnconfigure(2, weight=1, uniform="sample")
root.grid_columnconfigure(3, weight=2, uniform="sample")
...
more_info_menu.add_command(label="To Cite", command= to_reference)

# EFEL and IPFX package?
menubar.add_cascade(label="More Info...", menu=more_info_menu)

root.config(menu=menubar)

#! ------------------------------ Populate the GUI ------------------------------
# title = ttk.Label(root, text="G.A.E.A", style = "TopHeader.TLabel")
# title.grid(row=0, column=0, sticky = W, pady = (5,3), padx=(10, 10))
...
#? Configure Analysis ------------------------------------------------------------------------
configure_analysis_title = ttk.Label(root, text="Configure Analysis settings", style = "MidHeader.TLabel")
configure_analysis_title.grid(row=4, column=0, padx=(10, 10), sticky = W, pady = 10)
...
data_rows = 9
dataset_title = ttk.Label(root, text="Datasets to generate", style = "MidHeader.TLabel")
...
ap_table_button.grid(row= data_rows + 1, column=1, padx = (30,0))
...
ap_window_config.grid(row=data_rows + 1, column=2, sticky = W)
...
ap_table_file_name.grid(row= data_rows + 2, column=1, padx=(50, 0), sticky = W)
ap_table_file_name.insert(0,'Firing properties table')
...
ap_table_file_name_tp.grid(row=data_rows + 2, column=2, padx=(0, 10), sticky = W)
...
membrane_table_button = Checkbutton(root, text = "Passive membrane properties table", 
                            ...
                            font = ('calibri', 13),
                            background = 'white')
membrane_table_button.grid(row= data_rows + 3, column=1,padx = (30,0))
...
pm_window_config.grid(row=data_rows + 3, column=2, sticky = W)
...
membrane_table_file_name.grid(row = data_rows + 4, column=1, padx=(50, 0), sticky = W)
membrane_table_file_name.insert(0,'Passive membrane properties')
...
membrane_table_file_name_tp.grid(row=data_rows + 4, column=2, padx=(0, 10), sticky = W)
...
neuronal_overview_table_button.grid(row= data_rows + 5, column=1, padx = (30,0))
...
no_window_config.grid(row=data_rows + 5, column=2, sticky = W)
...
neuronal_overview_file_name.grid(row = data_rows + 6, column=1, padx=(50, 0), sticky = W)
neuronal_overview_file_name.insert(0,'Neuronal overview table')
...
neuronal_overview_file_name_tp.grid(row=data_rows + 6, column=2, padx=(0, 10), sticky = W)
neuronal_overview_file_name_icon = Hovertip(neuronal_overview_file_name_tp, neuronal_overview_file_name_text, hover_delay=50)


#--------- Graph Check Buttons ---------
graph_rows = 16
graph_title = ttk.Label(root, text="Graphs to generate", style = "MidHeader.TLabel")
graph_title.grid(row=graph_rows, column=0, padx=(10, 10), sticky = W, pady = 10)
...
graph_1_button = Checkbutton(root, text = "Firing-current plot", 
                            ...
                            font = ('calibri', 13),
                            background = 'white')
graph_1_button.grid(row=graph_rows+1, column=1, padx = (30,0)) 
...
graph_1_sample_image_tp.grid(row=graph_rows + 1, column=2, padx=(0, 10), sticky = W)
...
graph_2_button = Checkbutton(root, text = "Recording plot", 
                            ...
                            font = ('calibri', 13),
                            background = 'white')
graph_2_button.grid(row=graph_rows+2, column=1, padx = (30,0))
...
graph_2_sample_image_tp.grid(row=graph_rows + 2, column=2, padx=(0, 10), sticky = W)
...
graph_3_button = Checkbutton(root, text = "Protocol plot", 
                            ...
                            font = ('calibri', 13),
                            background = 'white')
graph_3_button.grid(row=graph_rows+3, column=1, padx = (30,0))
...
graph_3_sample_image_tp.grid(row=graph_rows + 3, column=2, padx=(0, 10), sticky = W)
...
graph_4_button = Checkbutton(root, text = "Current-voltage linear regression plot", 
                            ...
                            font = ('calibri', 13),
                            background = 'white')
graph_4_button.grid(row=graph_rows+4, column=1, padx = (30,0))
...
graph_4_sample_image_tp.grid(row=graph_rows + 4, column=2, padx=(0, 10), sticky = W)
...
reset_button = ttk.Button(text="Reset Values", 
                          ...
                          style ='Reset.TButton')
reset_button.grid(row=25, column=3, pady = (0,10), padx=(0,40), sticky = 'e')
...
status_title.grid(row=1, column=3, padx=(0,10), sticky = W)
...
if __name__ == '__main__':
    # This window now has the same size across all monitors. Notice that the scaled factor is one if the script is being run on a the same computer with ORIGINAL_DPI. 
    # root.geometry(f'{scaled(root.winfo_screenwidth() * 4/5)}x{scaled(root.winfo_screenheight()* 4/5)}')    
    root.geometry(f'{round(root.winfo_screenwidth() * 4 / 5)}x{round(root.winfo_screenheight() * 4 / 5)}')
    root.mainloop()